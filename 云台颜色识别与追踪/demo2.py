...
def color_track():
    ...
    #cv.resizeWindow("tracking",300,300)
    while True:
        ret, frame = cap.read()
        assert ret,print("摄像头开启失败")
        ...
        if len(cnts) > 0:
            ...
            if color_radius > 10:
                times += 1
                # 将检测到的颜色标记出来
                cv.circle(frame, (int(color_x), int(color_y)), int(color_radius), (255, 0, 255), 2)
        ...
        cv.putText(frame, "FPS " + str(int(mfps)), (40, 40), cv.FONT_HERSHEY_SIMPLEX, 0.8, (0, 255, 255), 3)
